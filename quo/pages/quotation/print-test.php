...
$items_stmt = $conn->prepare("
    SELECT qi.*, b.image, b.link_1, b.name_link_1, b.link_2, b.name_link_2, b.kategori 
...
foreach ($quote_items as $item) {
    if (isset($item['discount_value']) && (float)$item['discount_value'] > 0) {
...
$total_harga_bersih_produk = 0;
foreach ($quote_items as $item) {
    $gross_subtotal += (float)$item['item_price'] * (int)$item['quantity'];
    $subtotal_after_item_discounts += (float)$item['total_amount'];
    
    $harga_satuan = (float)$item['item_price'];
    $kuantitas = (int)$item['quantity'];
    $tipe_diskon = $item['discount_type'];
    $nilai_diskon = (float)$item['discount_value'];

    $harga_satuan_setelah_diskon = 0;

    if ($tipe_diskon == 'PERCENT') {
        $potongan_harga = $harga_satuan * ($nilai_diskon / 100);
        $harga_satuan_setelah_diskon = $harga_satuan - $potongan_harga;
    } elseif ($tipe_diskon == 'AMOUNT') {
        $harga_satuan_setelah_diskon = $harga_satuan - $nilai_diskon;
    } else {
        $harga_satuan_setelah_diskon = $harga_satuan;
    }

    $net_product_per_item = $harga_satuan_setelah_diskon * $kuantitas;

    $total_harga_bersih_produk += $net_product_per_item;
}
...
<script>
    const hasItemDiscounts = <?php echo $hasItemDiscounts ? 'true' : 'false'; ?>;
</script>
                        ...
                        <table class="table-bordered" style="color:black;">
                            <thead>
                                <tr class="text-center">
                                    ...
                                    <th class="col-desc-header" style="width: <?php echo $hasItemDiscounts ? '42%' : '52%'; ?>;">DESCRIPTION</th>
                                    <th class="col-picture" style="width: 10%;">PICTURE</th>
                                    <th class="col-qty-header" style="width: 8%;">QTY</th>
                                    <th class="col-price-header" style="width: 12%;">PRICE</th>
                                    <?php if ($hasItemDiscounts): ?>
                                    <th class="col-price-header" style="width: 8%;">DISC</th>
                                    <?php endif; ?>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center" style="text-transform:uppercase;"><?= htmlspecialchars($item['quantity']); ?> <?= htmlspecialchars($item['unit']); ?></td>
                                    ...
                                    <td class="text-center px-2">
                                        ...
                                        if ($item['discount_type'] == 'PERCENT') {
                                            echo rtrim(rtrim(number_format($item['discount_value'], 2, ',', '.'), '0'), ',') . '%';
                                        } elseif ($item['discount_type'] == 'AMOUNT' && (float)$item['discount_value'] > 0) {
                                            echo '<span class="float-start" style="font-size:8pt;">Rp</span>' . number_format($item['discount_value'], 0, ',', '.');
                                        } else {
                                            echo '0';
                                        }
                                        ?>
                                ...
                                <?php $no++; endforeach; ?>
                                <tr>
                                    <td colspan="<?php echo $hasItemDiscounts ? '6' : '5'; ?>" class="text-end px-2 colspan-toggle"><strong>Subtotal</strong></td>
                                    <td class="text-end px-2">
                                        <span class="float-start">Rp</span>
                                        <?= number_format($total_harga_bersih_produk, 0, ',', '.'); ?>
                                    </td>
                                </tr>
                                <?php if ($overall_discount_amount > 0): ?>
                                    <tr>
                                        <td colspan="<?php echo $hasItemDiscounts ? '6' : '5'; ?>" class="text-end px-2 colspan-toggle"><strong>Discount</strong></td>
                                        <td class="text-end px-2">
                                            <span class="float-start">Rp</span>
                                            <?= number_format($overall_discount_amount, 0, ',', '.'); ?>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                                <?php if ($quote['issuer'] === 'CV'): ?>
                                    <tr>
                                        <td colspan="<?php echo $hasItemDiscounts ? '6' : '5'; ?>" class="text-end px-2 colspan-toggle"><strong>PPN 11%</strong></td>
                                        <td class="text-end px-2">
                                            <span class="float-start">Rp</span>
                                            <?= number_format($ppn, 0, ',', '.'); ?>
                                        </td>
                                ...
                                <?php endif; ?>
                                <tr>
                                    <td colspan="<?php echo $hasItemDiscounts ? '6' : '5'; ?>" class="text-end px-2 colspan-toggle"><strong>Grand Total</strong></td>
...
document.getElementById('confirmPrintBtn').addEventListener('click', function() {
    ...
    document.querySelectorAll('.print-links').forEach(function(div) {
        div.style.display = includeLinks ? 'flex' : 'none';
    });

    let descWidth = hasItemDiscounts ? 42 : 55;
    let priceWidth = 12;
    let amountWidth = 13;
    ...
    document.querySelector('.col-desc-header').style.width = descWidth + '%';
    document.querySelector('.col-price-header').style.width = priceWidth + '%';
    document.querySelector('.col-amount-header').style.width = amountWidth + '%';

    let baseColspan = hasItemDiscounts ? 6 : 5;
    if (!includeImages) {
        baseColspan -= 1;
    }
    document.querySelectorAll('.colspan-toggle').forEach(function(cell) {
        cell.setAttribute('colspan', baseColspan);
    });
